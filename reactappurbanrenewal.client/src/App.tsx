...
function App() {
  ...
  const renderContent = () => {
    switch (activeTab) {
      ...
      case 'propertyValuation':
        return <PropertyValuation language={language} />;
      default:
        return <Dashboard language={language} />;
    }
  };
  ...
  const getVisibleMenuItems = () => {
    const allMenuItems = [
      ...
      { id: 'propertyValuation', icon: <Calculator size={20} />, label: t.propertyValuation, 
        visible: ['appraiser'].includes(userRole) },
  ...
  return (
    <div className={`min-h-screen bg-gray-100 ${isRTL ? 'rtl' : 'ltr'}`} dir={isRTL ? 'rtl' : 'ltr'}>
      ...
      {/* Sidebar */}
      <div 
        className={`fixed inset-y-0 ${isRTL ? 'right-0' : 'left-0'} z-50 bg-white shadow-lg transform transition-transform duration-300 ease-in-out ${
      ...
      <div 
        ...
        style={{
          marginLeft: window.innerWidth < 1024 ? 0 : isRTL ? 0 : sidebarCollapsed ? '5rem' : '16rem',
          marginRight: window.innerWidth < 1024 ? 0 : isRTL ? (sidebarCollapsed ? '5rem' : '16rem') : 0
        }}
      >
        {/* Desktop Header */}
